...
只有确认主机确实是 Ubuntu Noble、`ssh.socket` 与 generator 正在使用、`openssh-server` 已不低于 `1:9.6p1-3ubuntu13.13`，并且 `/etc/systemd/system/ssh.socket.d/override.conf` **仅包含旧的 `ListenStream=22`/`ListenStream=自定义端口` workaround** 时，才可让 generator 接管。先通过云控制台保留恢复入口，再运行下面的 fail-closed migration：
