...
async function visualizeSolve() {
  ...
  try { if (await solveStep(0, solveAbortController.signal)) {
    ...
    <div class="header">
      <div class="info-group">
        <div class="difficulty-select">
          <span 
            v-for="diff in ['Easy', 'Medium', 'Hard']" :key="diff"
            :class="{ active: difficulty === diff }"
            @click="initGame(diff as Difficulty)"
          >{{ diff }}</span>
        </div>
        <div class="stats">
...
.sudoku-container {
  ...
  gap: 20px; margin-top: 20px; font-family: sans-serif;
  user-select: none; touch-action: manipulation;
}

.header, .controls { width: 100%; max-width: 360px; }
...
.difficulty-select {
  display: flex; gap: 4px; background: var(--vp-c-bg-soft); padding: 4px; border-radius: 8px;
}
.difficulty-select span {
  padding: 4px 10px; border-radius: 6px; cursor: pointer; font-size: 0.8rem;
}
.difficulty-select span.active { background: var(--vp-c-brand); color: white; font-weight: bold; }
.stats .danger { color: #ef4444; font-weight: bold; }

.board-container {
  width: 100%; max-width: 360px;
  background: var(--vp-c-text-1); /* 粗边框颜色 */
  padding: 2px; /* 外框粗细 */
  border-radius: 4px;
}
...
.big-box {
  ...
  grid-template-rows: repeat(3, 1fr);
  gap: 1px; /* 单元格之间的细线宽度 */
  background-color: var(--vp-c-text-3); /* 细线颜色 */
}

/* 单元格 */
.cell {
  background-color: var(--vp-c-bg); /* 单元格背景色 */
  aspect-ratio: 1;
...
.cell.fixed { font-weight: 700; color: var(--vp-c-text-1); }
.cell:not(.fixed) { color: var(--vp-c-brand); }
.cell.error { background: #fecaca !important; color: #ef4444 !important; }
.dark .cell.error { background: #7f1d1d !important; }
.cell.related { background: var(--vp-c-bg-soft); }
.cell.same-num { background: #b1d2f9; }
.dark .cell.same-num { background: #1e3a8a; }
.cell.selected { background: #60a5fa !important; color: white !important; }
...
.note-num {
  ...
  color: var(--vp-c-text-2); line-height: 1;
}

.action-row { display: flex; gap: 8px; margin-bottom: 15px; }
.action-btn {
  flex: 1; padding: 10px; border-radius: 8px;
  border: 1px solid var(--vp-c-divider); background: var(--vp-c-bg-alt);
  font-size: 0.9rem; cursor: pointer; transition: all 0.2s;
}
.action-btn.active { background: var(--vp-c-brand); color: white; border-color: var(--vp-c-brand); }
.action-btn.special { color: #f59e0b; border-color: #f59e0b; }

.numpad { display: grid; grid-template-columns: repeat(9, 1fr); gap: 4px; }
.num-btn {
  aspect-ratio: 1; border-radius: 6px; border: 1px solid var(--vp-c-divider);
  background: var(--vp-c-bg-soft); color: var(--vp-c-brand);
...
.btn-primary {
  background: var(--vp-c-brand); color: white; border: none; padding: 10px 24px;
  border-radius: 6px; margin-top: 15px; cursor: pointer;
}
.loading-placeholder { height: 350px; display: flex; align-items: center; justify-content: center; }

@media (max-width: 450px) {
  .board-container, .header, .controls { max-width: 95vw; }
