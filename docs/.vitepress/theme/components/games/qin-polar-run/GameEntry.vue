...
import { FIELD as F, FRAME_LENGTH, type Phase, type WasmRunner } from './config'
...
function disposeRuntime() {
  stopLoop(); input?.dispose(); input = undefined
...
async function boot() {
  ...
  try {
    ...
    if (frame[0] !== 1) throw new Error('Unsupported runner ABI')
...
function tick(now: number) {
  ...
  if (frame[F.events] & 2) { feedback.value = '稳住，继续前进'; feedbackUntil = frame[F.time] + 1.8 }
  else if (frame[F.events] & 1 && frame[F.time] > feedbackUntil) { feedback.value = '+50'; feedbackUntil = frame[F.time] + 0.6 }
  if (frame[F.time] > feedbackUntil) feedback.value = ''
  scene.draw(frame, dt)
  if (frame[F.phase] === 3) {
    phase.value = 'over'; feedback.value = ''; input?.clear(); previous = 0; void focusOverlay()
  } else raf = requestAnimationFrame(tick)
}
function start() {
  if (!core || !scene) return
  stopLoop(); core.reset(seed()); input?.clear(); core.advance(0, 0, frame)
  score.value = 0; feedbackUntil = 0; feedback.value = ''; phase.value = 'running'
  surface.value?.focus({ preventScroll: true }); scene.draw(frame, 0)
  raf = requestAnimationFrame(tick)
}
function pause() {
  if (phase.value !== 'running' || !core) return
  stopLoop(); core.pause(true); input?.clear(); phase.value = 'paused'; feedback.value = ''; void focusOverlay()
}
function togglePause() {
  if (phase.value === 'running') pause()
  else if (phase.value === 'paused' && core && !document.hidden) {
    core.pause(false); input?.clear(); phase.value = 'running'; previous = 0
...
<template>
  <section ref="surface" class="northern-run" :class="{ 'is-running': phase === 'running' }"
    ...
    <p v-if="feedback && phase === 'running'" class="nr-feedback" role="status">{{ feedback }}</p>
    ...
    <div v-if="phase === 'ready'" class="nr-intro">
      ...
      <div class="nr-start"><button class="nr-primary" @click="start">开始奔跑 <span>↗</span></button>
        <p class="nr-controls"><span class="nr-keyboard">← → / A D 换道 · ↑ / Space 跳跃 · ↓ 俯身</span>
          <span class="nr-touch">左右滑动换道 · 上滑跳跃 · 下滑俯身</span></p></div>
