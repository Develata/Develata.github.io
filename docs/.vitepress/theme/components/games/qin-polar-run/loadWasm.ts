...
import { ABI_VERSION, type WasmPackage } from './config'
...
async function initialize(): Promise<WasmPackage> {
  if (!packageRequest) {
    ...
    asset.searchParams.set('abi', String(ABI_VERSION))
  ...
  await wasm.default({ module_or_path: withBase('/game-assets/qin-polar-run/wasm/qin_polar_run_bg.wasm') + '?abi=' + ABI_VERSION })
  return wasm
}
